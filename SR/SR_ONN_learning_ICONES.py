...
#LOADING, PATCHING, AND PREPARING ICONES DATASET
################################################################################
# get image tiles
list_ims = glob.glob("D:/HSI_SR_datasets/ICONES-Mix/*.hdr")
print(len(list_ims))
patch_size = 64
ctr = 1
hr_tiles = []
for i in list_ims:  
    print("Loading image number ", ctr)
    img = sp.open_image(i)
    hsi = hsi_normalize_full(img.load())
    elim_bands =  list(range(5))+ [30, 31, 32, 76, 95] + list(range(101,117)) + list(range(149,178))+ list(range(210,224))
    hsi = np.delete(img.load(),elim_bands, axis = 2) 
...
in_chans = 155
out_chans = 155
bs=64 #reduced from 64 because of memory issues  for ICONES with high order
ks=3
...
#ONN_model =WDSR_ONN(in_chans, out_chans, ks=ks, q_order=q_ord)
...
for trial in range(0,1):
    ...
    torch.save(model.state_dict(), 'D:/ONN/Self-ONN-main/ICONES/qord'+str(q_ord)+'_'+str(trial)+'_huber_3DWDSR'+'.pth')
